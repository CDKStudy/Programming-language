...
class PartBTransformsTest < Test::Unit::TestCase
  ...
  def test_image
    ...
    # check_transform(Image.new(uri), 0, 0)
    # check_transform(Image.new(uri, x: 0.1), 0.1, 0)
    # check_transform(Image.new(uri, y: 0.2), 0, 0.2)
    check_transform(Image.new(uri, x: 0.3, y: 0.4), 0.3, 0.4)
  end
