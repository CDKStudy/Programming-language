...
structure UW2Testing :> sig
    ...
    fun test_remove_card() =
        let
            ...
            fun assert_remove_card_raises_exception(crds, crd, e) = 
                ExceptionTesting.assertRaisesExceptionWithMessage(
                    fn() => remove_card(crds, crd, e), 
                    e, 
                    "remove_card(" ^ CardTesting.toStringFromList(crds) ^ ", " ^ CardTesting.toString(crd) ^ ", " ^ exnName(e) ^ ")"
                )
    ...
    fun test_officiate() =
        let
            ...
            fun assert_officiate_raises_exception(draw_pile, moves, goal) = 
                ExceptionTesting.assertRaisesExceptionWithMessage(
                    fn() => officiate(draw_pile, moves, goal), 
                    IllegalMove, 
                    "officiate(" ^ CardTesting.toStringFromList(draw_pile) ^ ", " ^ MoveTesting.toStringFromList(moves) ^ ", " ^ Int.toString(goal) ^ ")"
        ...
        in
            ( enter("officiate")
                ...
                ; assert_officiate_raises_exception([(Clubs,Jack),(Spades,Num(8))], [Draw,Discard(Hearts,Jack)], 42)
