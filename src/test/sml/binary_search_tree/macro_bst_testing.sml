structure IntBstTesting = BstTestingFn(struct
    ...
    val bst_to_dot_create_bst_params_text = "Int.compare, fn(v)=>v"
    val bst_to_dot_element_to_string_function_text = "Int.toString"
    val bst_to_dot_key_to_string_function_text = bst_to_dot_element_to_string_function_text
end)
...
structure StringBstTesting = BstTestingFn(struct
    ...
    val bst_to_dot_create_bst_params_text = "String.compare, fn(v)=>v"
    val bst_to_dot_element_to_string_function_text = "fn(s)=>s"
    val bst_to_dot_key_to_string_function_text = bst_to_dot_element_to_string_function_text
end)

structure StringIntEntryBstTesting = BstTestingFn(struct
	type entry = StringIntEntryTesting.t
	type key = string
	val compare_keys = String.compare
	fun to_key(k,v) = k
	val to_string_from_key = StringTesting.toString
	val to_string_from_entry = StringIntEntryTesting.toString
    val bst_to_dot_create_bst_params_text = "String.compare, fn(k,v)=>k"
    val bst_to_dot_element_to_string_function_text = "fn(s,n)=>s^\",\"^Int.toString(n)"
    val bst_to_dot_key_to_string_function_text = "fn(s)=>s"
end)
...
structure MacroBstTesting = struct
	...
	fun test_insert_equal_key() =
		let
			val _ = UnitTesting.enter("test_insert_equal_key")
			val bst = StringIntEntryBstTesting.assert_create_empty_completion()

			val _ = UnitTesting.enter("insert((\"X\", 425)")
			val _ = StringIntEntryBstTesting.assert_find(NONE, "bst", bst, "X")
			val bst = StringIntEntryBstTesting.assert_insert(NONE, "bst", bst, ("X", 425))
			val _ = StringIntEntryBstTesting.assert_find(SOME ("X", 425), "bst", bst, "X")
			val _ = StringIntEntryBstTesting.assert_bst_to_list([("X", 425)], "bst", bst)
			val _ = UnitTesting.leave()

			val _ = UnitTesting.enter("insert((\"X\", 231)")
			val _ = StringIntEntryBstTesting.assert_find(SOME("X", 425), "bst", bst, "X")
			val bst = StringIntEntryBstTesting.assert_insert(SOME("X", 425), "bst", bst, ("X", 231))
			val _ = StringIntEntryBstTesting.assert_find(SOME("X", 231), "bst", bst, "X")
			val _ = StringIntEntryBstTesting.assert_bst_to_list([("X", 231)], "bst", bst)
			val _ = UnitTesting.leave()
		in
			UnitTesting.leave() 
		end
		...
		( UnitTesting.enter("bst")
			...
			; ( 
				...
				then 
					...
					; name_tree_test(is_remove_testing_desired)
					)
				else ()
			)
		; UnitTesting.leave() )
