functor BstTestingFn (
    BstTestingParam : sig 
        ...
        val bst_to_dot_create_bst_params_text : string
        val bst_to_dot_element_to_string_function_text : string
        val bst_to_dot_key_to_string_function_text : string
    end
) : sig
    ...
    fun to_debug(inserts, removes) = 
        let
            val create_text = "val bst = BinarySearchTree.create_empty(" ^ bst_to_dot_create_bst_params_text ^ ")\n" 
    ...
    fun output_debug(xs) =
        print("\n\nv v v text for debug v v v \n\n" ^ to_debug(xs) ^ "\nval element_to_string = " ^ bst_to_dot_element_to_string_function_text ^ "\nval key_to_string = " ^ bst_to_dot_key_to_string_function_text ^ "\n\n^ ^ ^ text for debug ^ ^ ^ \n\n\n")
    ...
    fun assertInsertAllInOrderFollowedByRemove(entries: entry list, entry_to_remove : entry) : unit = 
        let
            ...
            val _ = assert_bst_to_list(expected_entries_after_remove, "bst_after_remove", bst') handle e => (output_debug(entries, [entry_to_remove]); raise e)
            fun f(entry) = 
