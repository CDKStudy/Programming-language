...
require_relative '../../../main/ruby/text_file_yield_and_rescue/text_file_yield_and_rescue'

class ContrivedError < StandardError
end

class TextFileYieldAndRescueTest < Test::Unit::TestCase
  def test_closed_after_success_with_block
    ...
    result = TextFileYieldAndRescue.write_text(file_path) do |file|
  ...
  def test_closed_after_error_with_block
    ...
    file_path = file_path_delete_if_necessary
    begin
      TextFileYieldAndRescue.write_text(file_path) do |file|
        file_passed_to_block = file
        assert_false(file.closed?)
  ...
  def test_open_without_block
    file_path = file_path_delete_if_necessary
    result = TextFileYieldAndRescue.write_text(file_path)
